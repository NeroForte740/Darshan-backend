...
class FuncionariosController {
  ...
  async create(req, res) {
    const { func_id, func_name, func_email, func_password, func_level } = req.body;

    if (!func_name || !func_email) {
      return res.status(400).json({ error: "Falta de dados importantes" });
    }

    const funcionarioExists = await FuncionariosRepository.findByName(func_name);

    if (funcionarioExists) {
      return res.status(400).json({ error: "Esse funcionario ja está cadastrado" });
    }
